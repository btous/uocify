  ...
  <main class="page-register">
    <section class="box-auth">
      ...
      <form>
        ...
        <div v-if="showSuccess" class="message success">{{ successMessage }}</div>
        <div v-if="showError" class="message error">{{ errorMessage }}</div>

        <button @click="register" class="btn btn-primary btn-lg">Registrar</button>
      </form>

      <p class="auth-bottom">
        ¿Ya tienes cuenta? <router-link to="/login" class="alink">Inicia sesión</router-link>
      </p>
    </section>
  </main>
</template>
  ...
  methods: {
    register: function (e) {
      ...
      firebase
        ...
        .then(
          (resolve) => {
            console.log('Nou usuari creat amb el correu: '+resolve.user.email);
            this.logged = true;
            this.showSuccess = true;
            this.successMessage = 'Se ha creado el usuario';
